      ...
      hammer.on("hammer.input", (ev: HammerInput) => {
        if (metricsRef.current) {
          if (Math.abs(ev.deltaX) > Math.abs(ev.deltaY)) {
            if (
              (ev.deltaX > 0 && currentPage !== 0) ||
              (ev.deltaX < 0 && currentPage + 1 !== pages)
            ) {
              (
                metricsRef.current.children[0] as HTMLDivElement
              ).style.transform = `translateX(${ev.deltaX}px)`;
            }
          }
          if (ev.isFinal) {
            (metricsRef.current.children[0] as HTMLDivElement).style.transform =
              "translateX(0)";
          }
        }
      });
