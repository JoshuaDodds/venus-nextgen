...
export const Batteries = observer(({ size }: BatteryProps) => {
  ...
  if (batteries) {
    if (battery) {
      ...
      const footer: Footer = {
        status: status,
        message: STATUS_LEVELS_MSG[status],
        property: "Charge",
      };
      return (
        <Card
          title={<Translate value="widgets.battery" />}
          size={size}
          footer={footer}
        >
          <div className={"battery"}>
            <div className={"battery__group " + size}>
              <div className="battery__charge">
                ...
                <div
                  className={
                    "battery__charge__body" +
                    (batteryLevelBars === CELL_NUMBER ? " full" : "")
                  }
                >
                  {batteryLevelBars > 0 &&
                    Array.from(Array(batteryLevelBars).keys())
                      ...
                      .map((idx) => (
                        <div
                          className={
                            "battery__charge__body__cell" +
                            getClassname(idx, batteryLevelBars)
                          }
              ...
              <div
                className={
                  "indicator-main" +
                  (size.includes(SIZE_SHORT) ? "--small" : "")
                }
              >
                <div>
                  <NumericValue
