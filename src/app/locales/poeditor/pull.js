...
const getLanguages = async () => {
  try {
    ...
    console.error(
      `An error occured while fetching the languages of the project. Error:`,
      e
    );
  }
};
...
const getExportFileURL = async (language) => {
  try {
    const res = await api.post("projects/export", {
      language,
      type: "key_value_json",
    });
    ...
    console.error(
      `An error occured while getting the export file URL. Error:`,
      e
    );
  }
};
...
(async () => {
  (await getLanguages()).forEach(async (language) => {
    try {
      ...
      console.error(
        `An error occured while saving the exported translations for ${language}. Error:`,
        e
      );
    }
  });
