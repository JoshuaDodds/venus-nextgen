...
async function syncTerms(sync = false) {
  ...
  try {
    if (sync) {
      ...
      console.log(
        "Terms from the main language synced. Result:",
        res.data.result.terms
      );
    } else {
      const res = await api.post("/terms/add", { data: JSON.stringify(terms) });
      console.log(
        "Terms from the main language added. Result:",
        res.data.result.terms
      );
    }
  } catch (e) {
    console.error(
      "An error occured while trying to add/sync the terms from the main language. Error:",
      e
    );
  }
}
...
async function addLanguages() {
  try {
    ...
    const projectLanguages = res.data.result.languages.map(
      (language) => language.code
    );
    const localLanguages = getLanguages();
    const difference = localLanguages.filter(
      (language) => !projectLanguages.includes(language)
    );
    if (difference.length)
      console.log(
        "Adding the following languages to the POEditor project:",
        difference
      );
    difference.forEach(async (language) => {
      const res = await api.post("/languages/add", { language });
      console.log(
        `${language} to the POEditor project. Result:`,
        res.data.response
      );
    });
  } catch (e) {
...
function syncTranslations() {
  ...
  languages.forEach(async (language) => {
    ...
    try {
      ...
      console.log(
        `Translations for ${language} synced. Result:`,
        res.data.result.translations
      );
    } catch (e) {
      console.error(
        `An error occurred while trying to sync the translations from ${language}. Error:`,
        e
      );
    }
  });
