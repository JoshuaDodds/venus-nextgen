...
const translations: TranslationRecord = Object.fromEntries(
  LANGUAGES.map((language) => [
    language,
    require(`./languages/${fileNameOverrides[language] || language}.json`),
  ])
);

// override the language with the value of the lang URL parameter, if present
const languageOverride = (window.location.search.match(
  /[?&]lang=([a-zA-Z-_]{2,5})/
) || [])[1];
...
const setupLocale = async () => {
  setLocale(
    (process.env.REACT_APP_ENABLE_LANG_OVERRIDE === "true" &&
      languageOverride) ||
