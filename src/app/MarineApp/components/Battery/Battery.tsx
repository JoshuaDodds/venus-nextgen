  ...
  return (
    <div className="battery__paginator">
      ...
      >
        <img
          src={LIcon}
          className="battery__paginator-button"
          alt={"Pagination button left"}
        />
      </SelectorButton>
  ...
  return (
    <ListViewWithTotals
      ...
      subTitle={
        amount +
        " " +
        translate("widgets." + (amount > 1 ? "batteries" : "battery"))
      }
      child={true}
    >
      <ListRow>
        {paginate && (
          <Paginator
            ...
            pages={Math.ceil(amount / pageSize)}
          />
        )}
      </ListRow>
    </ListViewWithTotals>
...
const BatteryRowAdditionalInfo = (battery: Battery) => {
  const temperature_min_max =
    useCustomBatteryMetrics().min_cell_temp +
    "° / " +
    useCustomBatteryMetrics().max_cell_temp +
    "°";
  ...
  return (
    <MetricValues inflate>
      <div className="metrics__left text--subtitle-upper">
        ...
        <ColumnContainer>
          {useCustomBatteryMetrics().capacity_available} /{" "}
          {useCustomBatteryMetrics().capacity_installed} Ah <br />
          SoH: {useCustomBatteryMetrics().state_of_health}%
        </ColumnContainer>
        <ColumnContainer>
          {useCustomBatteryMetrics().modules_online} Modules Online
        </ColumnContainer>
      </div>
    </MetricValues>
  );
  ...
  render() {
    ...
    return (
      <div
        ...
        ref={this.ref}
        style={
          this.ref.current && batteries.some((b) => b.dummy)
            ...
            >
              {!battery.dummy && (
                <div className="battery__data">
                  <div className="battery__title-row text--subtitle-upper">
                    {battery.name}
                  </div>
...
export class Batteries extends Component<BatteriesProps, BatteriesState> {
  ...
  updatePageSize() {
    const pageSize =
      window.innerHeight < 500 ? 1 : window.innerHeight < 700 ? 2 : 3;
...
export const BatteriesWithData = observer(() => {
  ...
  if (batteries) {
    ...
    const sorted = batteries.slice().sort((a, b) => {
      ...
      if (
        a.state === BATTERY_STATE.CHARGING &&
        b.state !== BATTERY_STATE.CHARGING
      )
        return -1;
      if (
        a.state !== BATTERY_STATE.CHARGING &&
      ...
      if (
        a.state === BATTERY_STATE.DISCHARGING &&
        b.state === BATTERY_STATE.IDLE
      )
        return -1;
      if (
        a.state === BATTERY_STATE.IDLE &&
        b.state === BATTERY_STATE.DISCHARGING
      )
        return 1;
      if (
        a.state === BATTERY_STATE.DISCHARGING &&
