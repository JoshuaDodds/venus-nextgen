...
export const BatteryLevel = observer(({ battery }: BatteryLevelProps) => {
  ...
  const showTimetoGo =
    battery.state === BATTERY_STATE.DISCHARGING && battery.timetogo;
  const showSoc = battery.soc !== undefined && battery.soc !== null;

  let batteryLabelClass =
    typeof batteryStateLabel === "string"
      ? "-" + batteryStateLabel.toLowerCase()
      : "idle";
  ...
  return (
    <div className={"metrics__right battery-level" + batteryLabelClass}>
      {batteryStateLabel && (
        <span>
          <Translate
            data-test-id="batteryStatus"
            value={"common." + batteryStateLabel}
          />
        </span>
      )}
