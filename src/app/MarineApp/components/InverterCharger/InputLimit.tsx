...
export const InputLimit = observer(
  ({ onChangeInputLimitClicked, shorePowerInput }: InputLimitProps) => {
    const { currentLimit, currentLimitIsAdjustable } =
      useInputLimit(shorePowerInput);

    let isAdjustable = currentLimitIsAdjustable || 0;

    let formattedLimit = formatNumber({
    ...
    return (
      <div className="metric__current-input-limit">
        <div className="input-limit-header__wrapper">
          {formattedLimit != null && (
            <div className="text--large metric__current-input-limit__limit">
              {formattedLimit}
            </div>
          )}
