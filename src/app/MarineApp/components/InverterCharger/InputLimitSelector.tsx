...
const InputLimitSelector = observer(
  ({ inputId, onLimitSelected }: InputLimitSelectorProps) => {
    const { currentLimit, currentLimitMax, productId, updateLimit } =
      useInputLimitSelector(inputId);
    ...
    useEffect(() => {
      if (
        ...
        Number(amperageContainerNode.current?.clientHeight) >
          Number(firstSelectorButtonNode.current?.clientHeight) * 2
      ) {
    ...
    const amperageList = getSuggestedAmperageValuesList(productId).filter(
      (value) => {
      ...
      <div className="amperage-selector__container">
        <div className="amperage-selector" ref={amperageContainerNode}>
...

          {
            ...
            showEmpties &&
              amperageList.map((amperage) => (
                <div className="empty" key={amperage} />
              ))
          }
        </div>
      </div>
