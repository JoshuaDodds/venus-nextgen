...
describe("DC Loads", () => {
  describe("when valid", () => {
    ...
    it("shows current", () => {
      ...
      expect(props.value).toEqual(
        98.765 / 12.345,
        "Current should be voltage divided by power"
      );
      expect(props.precision).toEqual(
        1,
        "Current should be displayed with one value"
      );
    });
  ...
  describe("when voltage is empty", () => {
    ...
    it("shows empty current", () => {
      expect(
        wrapper.find("NumericValue").first().props().value
      ).toBeUndefined();
    });
  });
