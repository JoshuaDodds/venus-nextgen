...
const getRequiredCols = (metrics: HTMLDivElement[]) => {
  ...
  return getMetricsTotalHeight(metrics) >
...
const getContainerHeight = (metrics: HTMLDivElement[]) => {
   ...
   */
  const metricsHeights = metrics.map(
    (c: HTMLDivElement) => c.getBoundingClientRect().height
  );

  const reversed = [...metricsHeights].reverse();
  const minHeight =
    metricsHeights.reduce((a: number, b: number) => a + b, 0) / 2;
  for (let i = 1; i <= metricsHeights.length; i++) {
    const leftColHeight = metricsHeights
      .slice(0, i)
  ...
  ({
    ...
    const computeResponsiveness = () => {
      if (metricsRef.current) {
        ...
        const newHeight =
          columns === 2
            ? Math.min(getContainerHeight(metrics)!, maxHeight)
            : maxHeight;
        setHeight(newHeight);

        const newPages = getRequiredPages(metrics, columns, newHeight);
        // set pages only if widgets are visible
        if (newPages !== pages && !visibleWidgetsStore.noVisibleElements)
          setPages(newPages);
      }
    };
